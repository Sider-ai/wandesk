...
export function AgentsTab({ data, loading }: { data: any; loading?: boolean }) {
  return (
    <GenericShell title="__T_CODEWORKSPACE_SUBAGENTS_TITLE__" subtitle={`__T_CODEWORKSPACE_SUBAGENTS_SUBTITLE__ · ${data?.total || ""}`}>
      {loading || !data ? <EmptyText text="__T_COMMON_LOADING__" /> : !data.available ? <EmptyText text={data.error || "-"} bad /> : (
          ...
          {(data.agents || []).map((agent: any) => (
            <div key={agent.name} className="cc-chart-card grid items-center gap-3" style={{ gridTemplateColumns: "48px 1fr", padding: "12px 16px" }}>
              ...
              <div>
                ...
                <div className="mt-0.5 text-[11px]" style={{ color: "#6b5a46" }}>{agent.group || "__T_CODEWORKSPACE_BUILT_IN__"}</div>
              </div>
            </div>
          ))}
        </div>
