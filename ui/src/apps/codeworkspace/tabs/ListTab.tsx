...
export function ListTab({ title, subtitle, data, loading, kind }: { title: string; subtitle?: string; data: any; loading?: boolean; kind?: string }) {
  ...
  return (
    <GenericShell title={title} subtitle={subtitle}>
      {loading || !data ? <EmptyText text="__T_COMMON_LOADING__" /> : data.error || data.available === false ? <EmptyText text={data.error || "__T_CODEWORKSPACE_UNAVAILABLE__"} bad /> : Array.isArray(items) && items.length ? (
      ...
      ) : data.empty ? <EmptyText text="__T_CODEWORKSPACE_NO_ITEMS__" /> : <JsonDetails data={data} />}
