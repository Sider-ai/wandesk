  ...
  return (
    <div className="passbook-table-wrapper mx-3 mb-4 flex-1 overflow-y-auto rounded border border-[rgba(82,113,255,0.4)] bg-white/40 shadow-[0_2px_10px_rgba(0,0,0,0.02)] sm:mx-10 sm:mb-10 sm:ml-[60px]">
      <table className="dot-matrix w-full border-collapse">
        <thead>
          <tr>
            <th className="sticky top-0 z-10 w-[12%] border border-[rgba(82,113,255,0.4)] bg-[rgba(82,113,255,0.15)] px-2 py-3 text-center text-xs font-bold text-[#1a2a40] backdrop-blur-[4px] sm:text-sm">{ "__T_FINANCE_COL_DATE__" }</th>
            <th className="sticky top-0 z-10 border border-[rgba(82,113,255,0.4)] bg-[rgba(82,113,255,0.15)] px-2 py-3 text-center text-xs font-bold text-[#1a2a40] backdrop-blur-[4px] sm:text-sm">{ "__T_FINANCE_COL_SUMMARY__" }</th>
            <th className="sticky top-0 z-10 w-[15%] border border-[rgba(82,113,255,0.4)] bg-[rgba(82,113,255,0.15)] px-2 py-3 text-center text-xs font-bold text-[#1a2a40] backdrop-blur-[4px] sm:text-sm">{ "__T_FINANCE_COL_EXPENSE__" }</th>
            <th className="sticky top-0 z-10 w-[15%] border border-[rgba(82,113,255,0.4)] bg-[rgba(82,113,255,0.15)] px-2 py-3 text-center text-xs font-bold text-[#1a2a40] backdrop-blur-[4px] sm:text-sm">{ "__T_FINANCE_COL_DEPOSIT__" }</th>
            <th className="sticky top-0 z-10 w-[10%] border border-[rgba(82,113,255,0.4)] bg-[rgba(82,113,255,0.15)] px-2 py-3 text-center text-xs font-bold text-[#1a2a40] backdrop-blur-[4px] sm:text-sm">{ "__T_FINANCE_COL_ACTION__" }</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.id} className="group transition-colors hover:bg-[rgba(82,113,255,0.05)]">
              ...
              <td className={`${cellClass} text-center`} onDoubleClick={() => startEdit(row, "note")}>
                {editInput(row, "note", "text-center") || <span>{row.note || (row.type === "income" ? "__T_FINANCE_DEFAULT_NOTE_DEPOSIT__" : "__T_FINANCE_DEFAULT_NOTE_EXPENSE__")}</span>}
              </td>
              ...
              <td className={`${cellClass} text-center`}>
                <button className="rounded border border-red-700 bg-transparent px-2 py-1 text-[11px] font-bold text-red-700 opacity-100 transition-all hover:bg-[#0b1c67] hover:text-white hover:shadow-[0_2px_4px_rgba(0,0,0,0.2)] active:translate-y-px sm:px-3 sm:text-xs sm:opacity-0 sm:group-hover:opacity-100" onClick={() => remove(row.id)}>{ "__T_FINANCE_ACTION_DELETE__" }</button>
              </td>
            </tr>
          ))}
          <tr className="bg-[rgba(82,113,255,0.08)]">
            ...
            <td className={`${cellClass} text-center`}><input value={form.newNote} onChange={(event) => setForm({ newNote: event.target.value })} type="text" className={`${inputClass} text-center`} placeholder="__T_FINANCE_LEDGER_NOTE_PLACEHOLDER__" onKeyDown={(event) => { if (event.key === "Enter") save(); }} /></td>
