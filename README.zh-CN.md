...
大多数 AI 产品和开源项目，仍然以对话作为主要交互方式。Wandesk 想探索的是另一种形态：一个本地桌面 / 工作台，把聊天、应用、文件、记忆、任务和代码 agent 放在同一个空间里。
...
在 Wandesk 里，创建一个应用应该更像是描述需求，而不是从零启动一个软件项目。

你可以打开 Chat 或 App Workshop，说出你想要的东西：

```text
创建一个轻量 CRM，用来记录客户、跟进、下一步动作和成交状态。
```

AI 工作流可以直接把这个需求落到代码里：
...
- 🔁 请求运行时重新加载，让新应用出现在桌面里

关键是，结果不是一次性的生成页面。它应该成为一个真正的 Wandesk 应用：可检查、可修改、本地运行，并且可以在后续对话中继续演化。
