...
export function JsonDetails({ data }: { data: any }) {
  ...
  return (
    <div className="mt-3">
      <button className="rounded-md px-2 py-1 text-[11px] hover:bg-black/5" style={{ color: "#5c4332" }} onClick={() => setOpen((value) => !value)}>{open ? "__T_CODEWORKSPACE_HIDE_RAW_JSON__" : "__T_CODEWORKSPACE_SHOW_RAW_JSON__"}</button>
...
export function InfoCard({ item }: { item: any }) {
  const name = item.name || item.id || item.pluginId || item.title || item.path || "__T_CODEWORKSPACE_ITEM__";
