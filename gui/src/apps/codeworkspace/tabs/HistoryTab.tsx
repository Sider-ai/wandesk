...
export function HistoryTab({ basePath, data, loading }: { basePath: string; data: any; loading?: boolean }) {
  ...
  return (
    <GenericShell title="__T_CODEWORKSPACE_HISTORY_TITLE__" subtitle={`__T_CODEWORKSPACE_FROM__ ${isCodex ? "~/.codex/history.jsonl" : "~/.claude/history.jsonl"} · ${data?.total || 0} __T_CODEWORKSPACE_ENTRIES_UNIT__`}>
      {loading || !data ? <EmptyText text="__T_COMMON_LOADING__" /> : !items.length ? <EmptyText text="__T_CODEWORKSPACE_HISTORY_EMPTY__" /> : (
          ...
          {items.map((item: any, index: number) => (
            <div key={`${item.sessionId || ""}-${index}`} className="flex gap-3 px-3 py-2" style={index < items.length - 1 ? { borderBottom: "1px solid rgba(140,100,60,0.06)" } : undefined}>
              ...
              <div className="min-w-0">
                <div className="text-[12.5px]" style={{ color: "#2a1f13" }}>{item.display || "__T_CODEWORKSPACE_EMPTY_VALUE__"}</div>
