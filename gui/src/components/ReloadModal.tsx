...
export default function ReloadModal() {
  ...
  const doReload = async () => {
    ...
    try {
      ...
      setPhase("error");
      setErrorMsg(error instanceof Error ? error.message : "__T_SHELL_RELOAD_UNKNOWN_ERROR__");
    }
  };
      ...
      <div className={`w-[420px] max-w-[calc(100vw-32px)] rounded-2xl border p-5 shadow-[0_24px_80px_rgba(0,0,0,0.28),0_2px_8px_rgba(0,0,0,0.08)] backdrop-blur-2xl ${panelClass}`}>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h3 className="text-[15px] font-bold">{"__T_SHELL_RELOAD_TITLE__"}</h3>
            <p className={`mt-0.5 text-[12px] leading-relaxed ${subtleText}`}>{"__T_SHELL_RELOAD_SUBTITLE__"}</p>
          </div>
        </div>
        ...
        <div className="mt-4 space-y-2 text-[12.5px]">
          <label className={`flex cursor-pointer items-center gap-2 rounded-xl border px-3 py-2.5 ${itemClass}`}>
            ...
            <span>{"__T_SHELL_RELOAD_REBUILD_FRONTEND__"}</span>
          </label>
          <label className={`flex cursor-pointer items-center gap-2 rounded-xl border px-3 py-2.5 ${itemClass}`}>
            ...
            <span>{"__T_SHELL_RELOAD_RESTART_APPS__"}</span>
          </label>
          <label className={`flex cursor-pointer items-center gap-2 rounded-xl border px-3 py-2.5 ${itemClass}`}>
            ...
            <span>{"__T_SHELL_RELOAD_RESTART_SERVER__"}</span>
          </label>
        </div>
        ...
        <div className="mt-5">
          {phase === "confirm" && (
            <div className="flex items-center justify-end gap-2">
              <button onClick={dismiss} className={`cursor-pointer rounded-xl border px-4 py-2 text-[13px] font-semibold transition ${secondaryButton}`}>{"__T_SHELL_RELOAD_CANCEL__"}</button>
              <button onClick={doReload} className={`cursor-pointer rounded-xl px-4 py-2 text-[13px] font-semibold shadow-[0_8px_24px_rgba(0,0,0,0.14)] transition active:scale-[0.98] ${primaryButton}`}>{"__T_SHELL_RELOAD_CONFIRM__"}</button>
            </div>
          )}
          ...
          {phase === "error" && (
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-end gap-2">
                <button onClick={dismiss} className={`cursor-pointer rounded-xl border px-4 py-2 text-[13px] font-semibold transition ${secondaryButton}`}>{"__T_SHELL_RELOAD_CLOSE__"}</button>
                <button onClick={doReload} className={`cursor-pointer rounded-xl px-4 py-2 text-[13px] font-semibold shadow-[0_8px_24px_rgba(0,0,0,0.14)] transition active:scale-[0.98] ${primaryButton}`}>{"__T_SHELL_RELOAD_RETRY__"}</button>
              </div>
            </div>
          )}
        </div>
