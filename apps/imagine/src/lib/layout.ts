...
export function sizeOf(_n: TreeNode): [number, number] {
  return VARIANT_SIZE;
}

export function layoutTree(nodes: TreeNode[]): Map<string, Pos> {
  ...
  function subH(id: string): number {
    const n = byId.get(id)!;
    const h = sizeOf(n)[1];
    const ks = kids(id);
  ...
  function place(id: string, cx: number, top: number) {
    const n = byId.get(id)!;
    const [w, h] = sizeOf(n);
    const total = subH(id);
...
export function boundsOf(pos: Map<string, Pos>) {
