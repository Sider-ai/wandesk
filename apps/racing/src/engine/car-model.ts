...
export function buildCar(paint: number, opt: { rim?: number } = {}): CarModel {
  ...
  const body = new THREE.Group();          // 车身(随加减速俯仰/侧倾)
  grp.add(body);
  ...
  // —— 车身:侧面轮廓拉伸(含轮拱开孔)——
  // 约束:轮拱顶(ARC_Y+ARC_R=0.78)必须低于该处腰线,否则轮廓自相交、三角化出脏几何
  const AX_F = 1.45, AX_R = -1.45, ARC_R = 0.44, ARC_Y = 0.34, BOT = 0.14;
  const dx = Math.sqrt(ARC_R*ARC_R - (ARC_Y-BOT)**2);   // 轮拱与底边交点
  const a0 = Math.PI + Math.asin((ARC_Y-BOT)/ARC_R), a1 = -Math.asin((ARC_Y-BOT)/ARC_R);
  const s = new THREE.Shape();
  s.moveTo(2.42, BOT);
  s.quadraticCurveTo(2.50, 0.18, 2.50, 0.30);     // 保险杠面
  s.quadraticCurveTo(2.49, 0.46, 2.34, 0.56);     // 鼻尖上沿
  s.quadraticCurveTo(2.05, 0.70, 1.75, 0.795);    // 鼻坡
  s.quadraticCurveTo(1.30, 0.845, 0.95, 0.86);    // 前翼子板顶(前拱处 ≈0.83)
  s.lineTo(-1.05, 0.875);                          // 腰线
  s.quadraticCurveTo(-1.70, 0.89, -2.05, 0.955);  // 鸭尾
  s.quadraticCurveTo(-2.42, 0.83, -2.38, 0.62);   // 尾面
  s.lineTo(-2.30, BOT);
  s.lineTo(AX_R - dx, BOT);
  s.absarc(AX_R, ARC_Y, ARC_R, a0, a1, true);     // 后轮拱
  s.lineTo(AX_F - dx, BOT);
  s.absarc(AX_F, ARC_Y, ARC_R, a0, a1, true);     // 前轮拱
  s.lineTo(2.42, BOT);
  ...
  cs.moveTo(1.02, 0.80);
  cs.quadraticCurveTo(0.62, 1.06, 0.18, 1.16);    // 风挡
  cs.quadraticCurveTo(-0.10, 1.215, -0.45, 1.215);// 车顶
  cs.quadraticCurveTo(-0.95, 1.17, -1.38, 0.985); // 溜背
  ...
  addBox(1.90,0.05,0.36, 0,0.125,2.42, carbonMat);            // 前铲
  addBox(1.00,0.17,0.08, 0,0.30,2.45, carbonMat);             // 进气格栅
  addBox(0.46,0.10,0.06, 0,0.485,2.435, carbonMat);           // 鼻面上格栅
  addBox(1.52,0.12,0.26, 0,0.19,-2.28, carbonMat);            // 扩散器
  for(const fx of [-0.45,-0.15,0.15,0.45])
    addBox(0.025,0.15,0.26, fx,0.115,-2.28, carbonMat);       // 扩散器鳍
  for(const sx of [-0.94,0.94]) addBox(0.07,0.10,1.9, sx,0.15,0, carbonMat); // 侧裙
  ...
  addBox(1.72,0.15,0.05, 0,0.74,-2.475, carbonMat);            // 尾灯底座(bevel 后尾面 ≈ -2.47)
  ...
  for(const sx of [-0.30,0.30]){
    ...
    flMat.color.setRGB(1.2, 2.4, 3.6);   // HDR 亮度,确保吃到 bloom
    const fl = new THREE.Mesh(new THREE.ConeGeometry(0.075,0.72,8), flMat);
    fl.rotation.x = -Math.PI/2;          // 锥尖朝后
